...
class WeatherApiClient {
  Future<Weather> getCurrentWeather(String cityName) async {
    //https://api.weatherapi.com/v1/current.json?key=5f1b28cd41784d3fbc7211358230106&q=London&aqi=no
    final url = '${Config().baseUrl}/${Config().currentUrl}?key=${Config()
        .apiKey}&q=$cityName';
    print(url);
    ...
    if (response.statusCode != 200) {
      print('Requests failed with status ${response.statusCode}');
      throw WeatherApiException('Error getting weather for $cityName');
    }
    final Map <String, dynamic> jsonResponse = Map.castFrom(
  ...
  Future<List<Forecast>> getWeeklyForecast(String cityName) async {
    //https://api.weatherapi.com/v1/forecast.json?key=5f1b28cd41784d3fbc7211358230106&q=London&days=7&aqi=no&alerts=no
    final url =
        '${Config().baseUrl}/${Config().forecastUrl}?key=${Config()
        .apiKey}&q=$cityName&days=7';
    print(url);
